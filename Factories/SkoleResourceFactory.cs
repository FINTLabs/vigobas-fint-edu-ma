...
{
    class SkoleResourceFactory
    {
        public static SkoleResource Create(IEmbeddedResourceObject skoleData)
        {
            ...
            if (values.TryGetValue(FintAttribute.systemId, out IStateValue skolenummerValue))
            {
                skolenummer = JsonConvert.DeserializeObject<Identifikator>(skolenummerValue.Value);
            }
            ...
            var Links = skoleData.Links;

            if (Links.TryGetValue(ResourceLink.organization, out IEnumerable<ILinkObject> organizationLinks))
            {               
                var organisasjon = new Link();
                string organizationLinkUri = LinkToString(organizationLinks);
                organisasjon = Link.with(organizationLinkUri);
                
                skoleResource.AddOrganisasjon(organisasjon);                
            }
